...
bool SSD1306_send_cmd_list(uint8_t *buf, int num) {
    for (int i=0;i<num;i++){
        if (SSD1306_send_cmd(buf[i]) == PICO_ERROR_GENERIC) return false;        
    }
    return true;
}
...
void createBitmapArea(struct render_area* area, uint8_t width, uint8_t height){
    area->start_page = 0;
    area->end_page = (height / SSD1306_PAGE_HEIGHT)  - 1;
    area->start_col = 0;
    area->end_col = width - 1;
    calc_render_area_buflen(area);
}

void drawBitmap(uint8_t* bitmap, struct render_area* area, uint8_t width, uint8_t height, uint8_t offset){
    area->start_col = offset;
    area->end_col = offset + width;
    render(bitmap, area);
}
...
void writeStringCenterInRestrictedArea(uint8_t *buf, int16_t x_left, int16_t y_top, int16_t x_right, int16_t y_bottom, char *text){
    WriteString(buf, x_left + (x_right-x_left-sizeof(text)*6)/2, y_top + (y_bottom-y_top-8)/2, text);
}
